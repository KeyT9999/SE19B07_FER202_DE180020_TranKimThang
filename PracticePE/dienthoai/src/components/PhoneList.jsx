  ...
  useEffect(() => {
    const fetchMobiles = async () => {
      try {
        ...
        setError(`Không thể tải danh sách mobiles. Vui lòng kiểm tra JSON Server có đang chạy trên port 3001 không. Lỗi: ${err.message}`);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <Container className="mt-4">
      ...
      {filteredMobiles.length === 0 ? (
        <Alert variant="info">No mobiles found.</Alert>
      ) : (
        <Row>
              ...
              <Col key={mobile.id} md={6} lg={4} className="mb-4">
                <Card className="h-100 shadow-sm" style={{ borderRadius: "12px", overflow: "hidden" }}>
                  ...
                  <Card.Body className="d-flex flex-column">
                    <Card.Title className="h5 mb-2">{mobile.name}</Card.Title>
                    <Card.Text className="flex-grow-1 mb-3" style={{ color: "#666", fontSize: "0.9rem" }}>
                      {mobile.description?.substring(0, 80)}...
                    </Card.Text>
                    <div className="mb-3">
