...
const applyFiltersAndSort = (payments, filters) => {
  ...
  if (filters.course) {
    data = data.filter((payment) => payment.course === filters.course);
  }
  ...
  switch (filters.sortBy) {
    case 'course_asc':
      ...
      break;
    case 'course_desc':
      data.sort((a, b) => (b.course || '').localeCompare(a.course || ''));
      break;
    case 'date_asc':
...
export const PaymentProvider = ({ children }) => {
  ...
  const availableCourses = useMemo(
    () => Array.from(new Set(state.payments.map((payment) => payment.course).filter(Boolean))),
    [state.payments]
  );
