...
import CameraAPI from "../api/CameraAPI";                       // Axios instance để gọi API
...
export function AuthProvider({ children }) {
  ...
  const fetchUser = useCallback(async () => {
    ...
    try {
      ...
      const response = await CameraAPI.get("/accounts");
  ...
  function login(identifier, password) {
    ...
    return new Promise((resolve) => {
      ...
      setTimeout(() => {
        ...
        if (!account) {
          // Dispatch action LOGIN_FAILURE → set error = "Invalid username or password!", isAuthenticated = false
  ...
  const contextValue = {
    ...state,        // Spread tất cả state (users, user, isAuthenticated, loading, error)
